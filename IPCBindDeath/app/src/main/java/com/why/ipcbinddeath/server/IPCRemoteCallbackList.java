 ...
 */
public class IPCRemoteCallbackList<E extends IInterface> {
    public ArrayMap<IBinder, Callback> mCallbacks
    ...
    /**
     * Simple version of {@link IPCRemoteCallbackList#register(E, Object)}
     ...
     *     } catch (RemoteException e) {
     *         // The MsdRemoteCallbackList will take care of removing
    ...
    public boolean containsCookie(@NonNull Object cookie){
      int i = beginBroadcast();
      boolean isContains = false;
      while (i > 0) {
          i--;
          if (cookie.equals(getBroadcastCookie(i))){
              isContains = true;
          }
      }
      finishBroadcast();
    ...
    public E get(Object cookie){
        E item = null;
        int i = beginBroadcast();
        while (i > 0) {
            i--;
            if (cookie.equals(getBroadcastCookie(i))){
                item = getBroadcastItem(i);
            }
        }
        finishBroadcast();
        return item;
